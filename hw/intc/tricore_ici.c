...
static uint64_t r(void*o,hwaddr a,unsigned z){TriCoreICIState*s=o;switch(a){case 0:return s->pending;case 4:return s->reset_mask;case 8:return s->provider;default:return 0;}}
static void w(void*o,hwaddr a,uint64_t v,unsigned z){TriCoreICIState*s=o; if(a==0){s->pending|=v&0x3f;for(unsigned i=0;i<6;i++)if(v&BIT(i))qemu_set_irq(s->irq[i],1);}else if(a==4)s->reset_mask=v&0x3f;else if(a==8)s->provider=v&0x3f;else if(a==0xc){s->pending&=~v;for(unsigned i=0;i<6;i++)if(v&BIT(i))qemu_set_irq(s->irq[i],0);}}
