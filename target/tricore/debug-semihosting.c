...
void helper_debug_semihost(CPUTriCoreState *env, uint32_t pc)
{
    int syscall;
    CPUState *cs = env_cpu(env);
    MemOpIdx oi = make_memop_idx(MO_UL | MO_LE, cpu_mmu_index(cs, true));
    uint32_t marker = cpu_ldl_code_mmu(env, pc - 4, oi, 0);
    ...
    if (marker != TRICORE_VIO_MARKER) {
        qemu_log("marker != TRICORE_VIO_MARKER\n");
        exit(0);
        return;
    }
